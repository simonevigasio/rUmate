...
router.get("/my-adv", auth, async (req, res) => {
    ...
    if (!ad) return res.status(400).send("There is no advertisement published by this user in the database");
...
router.get("/my-prefs", auth, async (req, res) => {
    ...
    if (!prefs) return res.status(400).send("There are no preferences related to the user in the database");
...
router.post("/", auth, async (req, res) => {
    ...
    if (!user) return res.status(400).send("Interested user id is not valid");
    
    // verify that the advertisement exists
    const ad = await Advertisement.findOne({_id: req.body.advertisement_id});
    if (!ad) return res.status(400).send("Advertisement does not exist");

    // verify whether the user is the owner of the advertisement
    if (ad.user_id == user._id) return res.status(400).send("The user is the onwer of the advertisement");
    ...
    if (prefs.length == 3) return res.status(400).send("The user has already 3 preferences")
    ...
    if (pref.length != 0) return res.status(400).send("The user has already signed the preference for this advertisement");
